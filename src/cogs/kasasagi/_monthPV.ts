    ...
    nt.storage.local.get(null).then((option)=>{
        if($(".novelview_h3").length){
            if(option.kasasagiCustomStyle){
                var m = $(".novelview_h3")
                var n = m.text().trim().match(/『(.*)』 月別\[全エピソード\] アクセス解析\(PV\)/)
                var title = ""
                if(n){
                    title = n[1]
                }
            ...
            if(option.kasasagiShowGraph_MonthPV){
                _graph(data.datasets, data.labels, option.kasasagiGraphType_MonthPV)
            }
...
function _button(datasets: Array<GraphDataset>, labels: Array<string>){
    ...
    if(ncode){
        ...
        $("#export-month-pv").on("click", function(){
            ...
            nt.download.json(raw, `month-pv_${ncode}_${date}.json`);
        })
    }
}
